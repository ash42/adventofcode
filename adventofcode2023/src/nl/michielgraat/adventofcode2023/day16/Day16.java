...
public class Day16 extends AocSolver {
     ...
     * @param loopList    A list which contains all pair of (x,y) coordinate &
     ...
     */
    private int getNrEnergizedTiles(int x, int y, char[][] grid, int direction, List<LoopDetectionKey> loopList,
        ...
        if (x < 0 || y < 0 || y >= grid.length || x >= grid[y].length || loopList.contains(key)) {
            memoMap.put(key, 0);
            return 0;
        }
        ...
        if (grid[y][x] == '.' || (grid[y][x] == '|' && (direction == UP || direction == DOWN))
            ...
            int nr = 1 + getNrEnergizedTiles(newX, newY, grid, direction, loopList, visitedList);
            memoMap.put(key, nr);
            return nr;
        } else if (grid[y][x] == '/') {
            ...
            int nr = 1 + getNrEnergizedTiles(newX, newY, grid, newDirection, loopList, visitedList);
            memoMap.put(key, nr);
            return nr;
        } else if (grid[y][x] == '\\') {
            int newX = (direction == LEFT || direction == RIGHT) ? x : (direction == UP) ? x - 1 : x + 1;
            ...
            int nr = 1 + getNrEnergizedTiles(x - 1, y, grid, LEFT, loopList, visitedList)
                    + getNrEnergizedTiles(x + 1, y, grid, RIGHT, loopList, visitedList);
            memoMap.put(key, nr);
            return nr;
        } else {
            // Current space is |. Direction should always be left or right, since the other
            // cases have already been covered by the first if.
            int nr = 1 + getNrEnergizedTiles(x, y - 1, grid, UP, loopList, visitedList)
                    + getNrEnergizedTiles(x, y + 1, grid, DOWN, loopList, visitedList);
            memoMap.put(key, nr);
            return nr;
        }
    }
    ...
    private int getNrUniqueEnergizedTiles(int x, int y, int direction, char[][] grid) {
        ...
        memoMap = new HashMap<>();
        int total = getNrEnergizedTiles(x, y, grid, direction, new ArrayList<>(), visited);
