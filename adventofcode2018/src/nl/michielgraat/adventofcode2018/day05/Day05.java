...
public class Day05 {
    ...
    private String react(final String input, final Character charToRemove) {
        String result = "";
        for (int i = 0; i < input.length(); i++) {
            final char cur = input.charAt(i);
            final char lcCur = Character.toLowerCase(cur);
            if (charToRemove == null || Character.toLowerCase(charToRemove.charValue()) != lcCur) {
                result += cur;
                if (result.length() >= 2) {
                    final char prev = result.charAt(result.length() - 2);
                    final char lcPrev = Character.toLowerCase(prev);
    ...
    public int runPart2(final List<String> lines) {
        ...
        for (char c = 'a'; c <= 'z'; c++) {
            final int length = react(lines.get(0), c).length();
            if (length < min) {
                min = length;
            }
        }
